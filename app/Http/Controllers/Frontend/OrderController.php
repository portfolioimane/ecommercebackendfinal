...
public function store(Request $request)
{
    ...
    $order = Order::create([
        ...
        'status' => 'pending',
        'shipping_area_id' => $request->input('shipping_area_id') ?? null, // Store shipping area ID if provided
    ]);
    ...
    // Retrieve the Pusher settings from the database
$pusherEnabled = Setting::where('type', 'pusher')->value('is_enabled');
         Log::info('pusher enabled:', [$pusherEnabled]);
...
switch ($request->input('payment_status')) {
    case 'paid':
        ...
        // Check if Pusher is enabled before broadcasting the event
        if ($pusherEnabled) {
    ...
    public function payment(Request $request)
{
    ...
    // Retrieve the Stripe API secret from the settings table
    $apiSecret = Setting::where('type', 'stripe')->where('key', 'api_secret')->value('value');

    // Check if the API secret is found
    if (!$apiSecret) {
        return response()->json(['error' => 'Stripe API secret not configured.'], 500);
    }
